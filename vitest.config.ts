  ...
  test: {
    ...
    coverage: {
      ...
      exclude: [
        ...
        // every `index.ts` this repository will ever hold, sight unseen. The
        // one file the wildcard actually described is
        // `apps/web/migrations/index.ts`, generated by `payload
        // migrate:create`, a bare array of the migration modules beside it
        // with zero authored logic; it is measured, at 100%, by
        // vitest.integration.config.ts, which DOES include
        // `apps/web/migrations/**` and now excludes it by this same exact
        // path. Against THIS config's `include` list the entry is inert today
        // - no glob above reaches `apps/web/migrations/**` - and it is kept
        // rather than deleted so that an `include` widened later re-excuses
        // that one generated file by name instead of re-introducing the
        // wildcard.
