 ...
 * drives it with plain objects. The script is the I/O shell around it.
 *
 ...
 *
 * @param {{ config: string, results: unknown }} input - `config` is the
 *   `lighthouserc*.json` the run used, named in every line so a reader knows
 *   which of three gates spoke. `results` is the parsed contents of
 *   `.lighthouseci/assertion-results.json`, or anything at all when that file
 *   was missing or unreadable — this function never assumes it parsed.
 ...
 *   that never happened.
 * @example
 * annotationLines({ config: 'lighthouserc.book.json', results })
 * // ['::error title=largest-contentful-paint lighthouserc.book.json::http://… expected 3000, measured 3212 (runs: 3100, 3212, 3240)']
 */
export const annotationLines = ({ config, results }) => {
  if (!Array.isArray(results)) {
    return [
      `::warning title=lighthouse-annotations ${config}::assertion results could not be read, so no measured number reached this log`,
    ]
  }
