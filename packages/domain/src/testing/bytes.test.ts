/**
 * bytes.test.ts — the byte-level fixtures' own tests.
 *
 * Split out of `factories.test.ts` alongside the fixtures themselves. Each
 * `describe` below asserts the ONE claim its fixture's docstring makes about
 * real files — that a JPEG opens with the three bytes the sniff keys on, that
 * PNG's signature carries its line-ending canary, that `ftyp` and a brand sit
 * at offsets 4 and 8, that the AVIF bytes are a real encoder's, that an SVG
 * root element survives a prologue and a namespace prefix, and that the EXIF
 * fixture's APP1 segment is laid out where its docstring says.
 *
 * WHAT THESE CASES ARE NOT. They do not certify that the bytes match a real
 * encoder's — a test in this package cannot, because `packages/domain`
 * depends on no encoder. `apps/web/lib/media/exifFixtureCertification.test.ts`
 * is that gate for `anExifJpeg`, and the `MediaProcessor` contract suite's
 * own fixture controls are it for the photographs.
 * Depends on: vitest, ./bytes.
 */
import { describe, expect, it } from 'vitest'
import { aJpegHeader, anAvifHeader, anExifJpeg, anIsoBmffHeader, anSvgDocument, aPngHeader, EXIF_CANARY } from './bytes'
