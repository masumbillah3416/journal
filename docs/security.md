...
| Sessions revocable, listed on the Account screen | real `sessions` rows (`docs/adr/0017-session-store-and-rotation.md`), read on every authentication by `apps/web/lib/auth/sessions.ts`'s `authenticate`, whose verdict comes from `sessionState` (`packages/domain/src/auth/session.ts`). `revokeSession` and `revokeAllSessions` back Revoke and "Sign out everywhere"; the Account screen's own path — a Payload `update` under the reader's own per-user access rule (`apps/web/collections/sessions.ts`, `docs/deviations.md` §29) — is asserted to reach the same row, so the two cannot drift apart. Every revocation case authenticates afterwards rather than asserting on `revokedAt`: a test that checks a field was written passes while nothing reads it | **Phase 2 Task 6** (rows, revocation, per-user access); Phase 4 (Account screen lists them) |
